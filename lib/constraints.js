    ...
    constraints: {
        ...
        per_host: {
            harmonize: function(core, options, fn){
                core.cluster.myriad.keys([core.constants.myriad.CONTAINERS_PREFIX, options.application_name, "*"].join("::"), function(err, containers){
                    var peers = _.groupBy(core.cluster.legiond.get_peers(), "mode");

                    var difference = options.application.tags.constraints.per_host * peers.follower.length - containers.length;
