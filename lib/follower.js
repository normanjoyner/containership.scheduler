    ...
    return {
        ...
        container: {
            start: function(configuration){
                core.cluster.myriad.persistence.get([core.constants.myriad.CONTAINERS_PREFIX, configuration.application, configuration.container_id].join("::"), function(err, container){
                    if(err)
                        core.loggers["containership.scheduler"].log("error", error.message);
            ...
            stop: function(options){
                engines[container.engine].stop(options);
            },
