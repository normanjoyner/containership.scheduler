...
module.exports = function(core){
    ...
    _.each(engines, function(engine, engine_name){
        engine.add_pre_start_middleware("codexd", function(options, fn){
            var volumes = _.groupBy(options.volumes, function(volume){
                return volume.host;
            });

            var codexd_volumes_metadata = {};

            async.each(volumes.undefined, function(volume){
                ...
                options.volumes = _.map(options.volumes, function(volume){
                    if(volume.host == undefined && _.has(codexd_volumes_inverse, volume.container)){
                        return {
                            host: [core.cluster.codexd.options.base_path, codexd_volumes_inverse[volume.container],
                ...
                if(!_.has(options.tags, "metadata"))
                    options.tags.metadata = {};

                if(!_.has(options.tags.metadata, "codexd"))
                    options.tags.metadata.codexd = {};

                if(!_.has(options.tags.metadata.codexd, "volumes")){
                    options.tags.metadata.codexd.volumes = codexd_volumes_metadata;
                    return fn();
                }
                else{
                    var codexd_errs = [];

                    async.each(_.keys(options.tags.metadata.codexd.volumes), function(uuid, fn){
                        ...
                        options.tags.metadata.codexd.volumes = _.defaults(options.tags.metadata.codexd.volumes, codexd_volumes_metadata);
                    }, function(){
                        core.cluster.myriad.persistence.set([core.constants.myriad.CONTAINERS_PREFIX, options.application_name, options.id].join(core.constants.myriad.DELIMITER), JSON.stringify(_.omit(options, "application_name")), function(err){
